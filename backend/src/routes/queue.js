...
router.get('/matches/:patientId', authenticate, requireRole('placement_agent', 'admin'), async (req, res, next) => {
  try {
    ...
    // Location filter (city OR zip OR county)
    ...
    // Budget filter — only apply if patient has a budget_max
    ...
    const matchQuery = `
      SELECT
        ...
        CASE WHEN cha.base_price_monthly IS NOT NULL AND cha.base_price_monthly BETWEEN $1::numeric * 0.8 AND $1::numeric * 1.2
             THEN 0 -- budget already filtered
             ELSE 0 END +
        CASE WHEN cha.gender_preference = 'any' OR cha.gender_preference IS NULL THEN 10 ELSE 0 END
        AS match_score,
        -- Existing proposal status for this patient
...
router.post('/matches/:patientId/select', authenticate, requireRole('placement_agent'), async (req, res, next) => {
  try {
    ...
    // Get match score for each care home from previous query
